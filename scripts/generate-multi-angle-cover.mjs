...
if (!API_KEY) {
  console.error("API key not found. Set NANO_BANANA_API_KEY or KIE_AI_API_KEY environment variable.");
  process.exit(1);
}
...
async function pollTaskStatus(taskId) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    const response = await fetch(`${API_BASE_URL}/jobs/getTaskStatus?taskId=${taskId}`, {
      method: "GET",
      headers: {
        "Authorization": `Bearer ${API_KEY}`,
      },
...
async function main() {
  try {
    ...
    const outputPath = "/home/ubuntu/nano-influencer/client/public/covers/multi-angle.jpg";
