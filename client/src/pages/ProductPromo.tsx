...
export default function ProductPromo() {
  ...
  const { data: videoStatus, refetch: refetchStatus } = trpc.productPromo.getStatus.useQuery(
    { videoId: currentVideoId! },
    { enabled: !!currentVideoId, refetchInterval: currentVideoId ? 5000 : false }
  );
  const creditsQuery = trpc.generation.getCredits.useQuery(undefined, { enabled: isAuthenticated });
  ...
  const handleGenerate = useCallback(() => {
    ...
    createVideoMutation.mutate({
      ...
      stylePreset: selectedStyle as "minimal_clean" | "premium_luxury" | "tech_futuristic" | "social_viral",
      productName: productName || undefined,
      slogan: slogan || undefined,
    });
  }, [productImage, selectedStyle, selectedStyleInfo, productName, slogan, userCredits, createVideoMutation]);
  ...
  const isVideoProcessing = videoStatus?.status === "processing" || videoStatus?.status === "pending";
        ...
        {!currentVideoId ? (
          ...
          <div className="max-w-4xl mx-auto space-y-8">
            ...
            <Card className="bg-zinc-900/50 border-zinc-800">
              <CardHeader>
                ...
                <CardDescription>
                  Tanıtım videosu oluşturmak istediğiniz ürünün net bir fotoğrafını yükleyin
                </CardDescription>
              </CardHeader>
              <CardContent>
                <input
                ...
                {!productImage ? (
                  <button
                    ...
                    className="w-full h-64 border-2 border-dashed border-zinc-700 rounded-xl hover:border-[#CCFF00]/50 transition-colors flex flex-col items-center justify-center gap-4"
                  >
                    {isUploading ? (
                      ...
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-3">
                          <div className={`p-2.5 rounded-lg ${
                            selectedStyle === style.id ? "bg-[#CCFF00]/20" : "bg-zinc-800"
                          }`}>
                            {styleIcons[style.id]}
                          </div>
                          <span className="font-bold text-base">{style.nameTr}</span>
                        </div>
                        <span className={`text-sm font-bold px-2 py-1 rounded-md ${
                          selectedStyle === style.id ? "text-[#CCFF00] bg-[#CCFF00]/10" : "text-zinc-400 bg-zinc-800"
                        }`}>
                          {style.credits} Kredi
                        </span>
                      </div>
                      <p className="text-sm text-zinc-400 pl-12">{style.description}</p>
                    </motion.button>
                  ))}
                </div>
              </CardContent>
            ...
            <Card className="bg-zinc-900/50 border-zinc-800">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <span className="w-6 h-6 bg-zinc-700 text-white rounded-full flex items-center justify-center text-sm font-bold">3</span>
          ...
          <div className="max-w-3xl mx-auto space-y-8">
            ...
            <Card className="bg-zinc-900/50 border-zinc-800">
              <CardContent className="py-8">
                <div className="text-center space-y-4">
                  {isVideoProcessing && (
                    <>
                      <div className="w-20 h-20 mx-auto bg-[#CCFF00]/10 rounded-full flex items-center justify-center">
                        <Loader2 className="w-10 h-10 text-[#CCFF00] animate-spin" />
                      </div>
                      <div>
                        <h2 className="text-2xl font-bold">Video Oluşturuluyor</h2>
                        <p className="text-zinc-400 mt-1">Bu işlem 1-3 dakika sürebilir</p>
                      </div>
                  ...
                  {isVideoCompleted && (
                    <>
                      ...
                      <div>
                        ...
                        <p className="text-zinc-400 mt-1">Videonuz başarıyla oluşturuldu</p>
                      </div>
                    </>
                  )}
              ...
              <Card className="bg-zinc-900/50 border-zinc-800 overflow-hidden">
                <CardContent className="p-0">
                  <video
                    ...
                    autoPlay
                    loop
                    className="w-full aspect-[9/16] max-h-[600px] object-contain bg-black"
                  />
                </CardContent>
              </Card>
            )}
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              {isVideoFailed && (
                <Button
                  onClick={() => retryMutation.mutate({ videoId: currentVideoId! })}
            ...
            {videoStatus?.productImageUrl && (
              <Card className="bg-zinc-900/50 border-zinc-800">
                <CardHeader>
                  <CardTitle className="text-sm text-zinc-400">Orijinal Ürün Görseli</CardTitle>
                </CardHeader>
                <CardContent>
                  <img
