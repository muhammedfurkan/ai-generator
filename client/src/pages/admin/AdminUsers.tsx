...
export default function AdminUsers() {
  ...
  const createUserMutation = trpc.adminPanel.createUser.useMutation({
    onSuccess: () => {
      ...
      setNewUserForm({ name: "", email: "", password: "", credits: 100, role: "user" });
    },
    onError: (error) => toast.error(error.message),
  });
                  ...
                  {paginatedUsers.map((user) => (
                    <tr
                      ...
                      className={`
                        ...
                        ${selectedUser === user.id ? "bg-lime-500/10" : ""}
                      `}
                      onClick={() => setSelectedUser(user.id)}
                    >
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${(user as any).isBanned
                            ? "bg-red-500/30 text-red-300 border border-red-500/50"
                            : "bg-gradient-to-br from-lime-400 to-emerald-500 text-black"
                            }`}>
                            {(user as any).isBanned ? <Ban className="h-4 w-4" /> : (user.name?.[0] || "?")}
                          </div>
                          <div>
                            <div className="flex items-center gap-2">
                              <p className={`font-medium ${(user as any).isBanned ? "text-red-300" : ""}`}>
                                {user.name || "İsimsiz"}
                              </p>
            ...
            {selectedUser && userDetailsQuery.data ? (
              ...
              >
                ...
                <div className="flex items-center gap-4 mb-6">
                  <div className="w-16 h-16 rounded-full bg-gradient-to-br from-lime-400 to-emerald-500 flex items-center justify-center text-black text-2xl font-bold">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-zinc-500">{userDetailsQuery.data.user.email}</p>
                    <span
                      className={`inline-block mt-1 px-2 py-0.5 rounded-full text-xs font-semibold ${userDetailsQuery.data.user.role === "admin"
                        ? "bg-red-500/20 text-red-400"
                        : "bg-zinc-500/20 text-zinc-400"
                        }`}
                    >
                      {userDetailsQuery.data.user.role === "admin" ? "Admin" : "Kullanıcı"}
                    </span>
                ...
                <div className="grid grid-cols-2 gap-3 mb-6">
                  <div className="bg-zinc-800/50 rounded-xl p-3 text-center">
                    <CreditCard className="h-5 w-5 mx-auto mb-1 text-blue-400" />
                  ...
                  <div className="bg-zinc-800/50 rounded-xl p-3 text-center">
                    <Image className="h-5 w-5 mx-auto mb-1 text-purple-400" />
                    <p className="text-lg font-bold">{userDetailsQuery.data.stats.totalImages}</p>
                    <p className="text-xs text-zinc-500">Görsel</p>
                  </div>
                  <div className="bg-zinc-800/50 rounded-xl p-3 text-center">
                    <Video className="h-5 w-5 mx-auto mb-1 text-pink-400" />
                    <p className="text-lg font-bold">{userDetailsQuery.data.stats.totalVideos}</p>
                    <p className="text-xs text-zinc-500">Video</p>
                  </div>
                  <div className="bg-zinc-800/50 rounded-xl p-3 text-center">
                    <Calendar className="h-5 w-5 mx-auto mb-1 text-green-400" />
                    <p className="text-lg font-bold">
                      {format(new Date(userDetailsQuery.data.user.createdAt), "d MMM", {
                        locale: tr,
                      })}
                    </p>
                ...
                <div className="space-y-2">
                  ...
                  <Button
                    className="w-full justify-start gap-2 bg-blue-500/20 text-blue-400 hover:bg-blue-500/30"
                    onClick={() => {
                      const newRole =
                        userDetailsQuery.data!.user.role === "admin" ? "user" : "admin";
                      if (
                        confirm(
                  ...
                  {(userDetailsQuery.data.user as any).isBanned ? (
                    <Button
                      className="w-full justify-start gap-2 bg-green-500/20 text-green-400 hover:bg-green-500/30"
                      onClick={() => {
                        if (confirm("Bu kullanıcının yasağını kaldırmak istiyor musunuz?")) {
                ...
                {userDetailsQuery.data.transactions.length > 0 && (
                  <div className="mt-6">
                    ...
                    <div className="space-y-2 max-h-40 overflow-y-auto">
                      {userDetailsQuery.data.transactions.slice(0, 5).map((tx) => (
                        <div
                          key={tx.id}
                        ...
                        >
                          <div className="flex items-center gap-2">
                            <div
                              className={`w-2 h-2 rounded-full ${tx.type === "add"
                                ? "bg-green-400"
                                : tx.type === "deduct"
                                  ? "bg-red-400"
                                  : "bg-blue-400"
                          ...
                          <span
                            className={`font-semibold ${tx.type === "add" ? "text-green-400" : "text-red-400"
                              }`}
                          >
                            {tx.type === "add" ? "+" : "-"}
                            {tx.amount}
      ...
      <Dialog open={creditDialogOpen} onOpenChange={setCreditDialogOpen}>
        <DialogContent className="bg-zinc-900 border-white/10">
          ...
          <div className="space-y-4 mt-4">
            ...
            <div>
              <label className="text-sm font-medium mb-2 block">Sebep (Opsiyonel)</label>
              <Input
            ...
            <div className="flex gap-3">
              ...
              <Button
                className={`flex-1 ${creditAction === "add"
      ...
      <Dialog open={banDialogOpen} onOpenChange={setBanDialogOpen}>
        <DialogContent className="bg-zinc-900 border-white/10">
          <DialogHeader>
            ...
            <DialogDescription>
              Bu kullanıcı sisteme giriş yapamayacak ve yeni hesap oluşturamayacak.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 mt-4">
            {selectedUser && userDetailsQuery.data && (
              <div className="p-4 bg-orange-500/10 border border-orange-500/20 rounded-lg">
                <p className="font-medium">{userDetailsQuery.data.user.name || "İsimsiz"}</p>
                <p className="text-sm text-zinc-500">{userDetailsQuery.data.user.email}</p>
              </div>
            )}
            <div>
              <label className="text-sm font-medium mb-2 block">Yasaklama Sebebi</label>
              <Textarea
        ...
        <DialogContent className="bg-zinc-900 border-white/10">
          ...
          <div className="space-y-4 mt-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-sm font-medium mb-2 block">Başlangıç Kredisi</label>
                <Input
                  type="number"
