...
export default function AdminJobQueue() {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case "processing": return <Play className="h-4 w-4 text-blue-400 animate-pulse" />;
      case "completed": return <CheckCircle className="h-4 w-4 text-green-400" />;
      case "failed": return <XCircle className="h-4 w-4 text-red-400" />;
      case "cancelled": return <Pause className="h-4 w-4 text-zinc-400" />;
  ...
  const getStatusBadge = (status: string) => {
    const config: Record<string, { color: string; label: string }> = {
      ...
      processing: { color: "bg-blue-500/20 text-blue-400", label: "İşleniyor" },
      completed: { color: "bg-green-500/20 text-green-400", label: "Tamamlandı" },
      failed: { color: "bg-red-500/20 text-red-400", label: "Başarısız" },
      cancelled: { color: "bg-zinc-500/20 text-zinc-400", label: "İptal" },
    };
    const c = config[status] || { color: "bg-zinc-500/20 text-zinc-400", label: status };
    return (
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case "upscale": return <Zap className="h-4 w-4 text-green-400" />;
      case "multi_angle": return <Layers className="h-4 w-4 text-orange-400" />;
      case "skin_enhancement": return <Zap className="h-4 w-4 text-pink-400" />;
      case "product_promo": return <Video className="h-4 w-4 text-cyan-400" />;
      case "ugc_ad": return <Video className="h-4 w-4 text-lime-400" />;
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="bg-zinc-800/50">
          <TabsTrigger value="internal" className="data-[state=active]:bg-lime-500 data-[state=active]:text-black">
            <Activity className="h-4 w-4 mr-2" />
            Internal Kuyruk
          </TabsTrigger>
          <TabsTrigger value="kie-api" className="data-[state=active]:bg-lime-500 data-[state=active]:text-black">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
            ...
            <div className="flex gap-3">
              ...
              <Button
                ...
                onClick={() => {
                  if (confirm("Tamamlanmış tüm işleri temizlemek istediğinizden emin misiniz?")) {
              ...
              <table className="w-full">
                <thead className="bg-zinc-800/50 border-b border-white/10">
                  <tr>
                    ...
                    <th className="text-left py-3 px-4 text-sm font-medium text-zinc-400">Tip</th>
                    ...
                    <th className="text-left py-3 px-4 text-sm font-medium text-zinc-400">Deneme</th>
                    <th className="text-left py-3 px-4 text-sm font-medium text-zinc-400">Süre</th>
                    <th className="text-left py-3 px-4 text-sm font-medium text-zinc-400">Tarih</th>
                    <th className="text-right py-3 px-4 text-sm font-medium text-zinc-400">İşlemler</th>
                  </tr>
                </thead>
                    ...
                    <tr key={job.id} className="hover:bg-zinc-800/50 transition-colors">
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p className="text-sm">{job.userName || `User #${job.userId}`}</p>
                            <p className="text-xs text-zinc-500">{job.userEmail}</p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="py-3 px-4 text-sm">
                        {job.actualDurationMs ? formatDuration(job.actualDurationMs) : "-"}
                      </td>
                      <td className="py-3 px-4 text-sm text-zinc-400">
                        {formatDistanceToNow(new Date(job.queuedAt), { locale: tr, addSuffix: true })}
                      </td>
                        ...
                        <div className="flex items-center justify-end gap-2">
                          ...
                          {(job.status === "failed" || job.status === "cancelled") && (
                            <Button
                              ...
                              onClick={() => retryJobMutation.mutate({ id: job.id })}
                ...
                {filteredJobs
                  ...
                  .map((job: any) => (
                    <div key={job.id} className="bg-zinc-900/50 rounded-lg p-3">
                      ...
                      <p className="text-sm text-red-300 font-mono">{job.lastError}</p>
                    </div>
                  ))}
              </div>
            </div>
        ...
        <TabsContent value="kie-api" className="mt-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              className="bg-gradient-to-br from-blue-500/20 to-blue-600/10 rounded-2xl border border-blue-500/30 p-4"
            >
              <div className="flex items-center gap-3">
                <Video className="h-5 w-5 text-blue-400" />
                <div>
                  <p className="text-lg font-bold">{kieTasks?.stats?.processingVideos || 0}</p>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-lg font-bold">{kieTasks?.stats?.pendingVideos || 0}</p>
            ...
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.2 }}
              className="bg-gradient-to-br from-purple-500/20 to-purple-600/10 rounded-2xl border border-purple-500/30 p-4"
            >
              <div className="flex items-center gap-3">
                <Image className="h-5 w-5 text-purple-400" />
                <div>
                  <p className="text-lg font-bold">{kieTasks?.stats?.processingImages || 0}</p>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-lg font-bold">{kieTasks?.stats?.pendingImages || 0}</p>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            >
              <RefreshCw className={`h-4 w-4 ${kieTasksQuery.isFetching ? "animate-spin" : ""}`} />
              Yenile
            </Button>
          </div>
                  ...
                  <table className="w-full">
                    <thead className="bg-zinc-800/50 border-b border-white/10">
                      <tr>
                        ...
                        <th className="text-left py-3 px-4 text-sm font-medium text-zinc-400">DB Durumu</th>
                        {checkLiveStatus && (
                          <th className="text-left py-3 px-4 text-sm font-medium text-zinc-400">API Durumu</th>
                        )}
                        <th className="text-left py-3 px-4 text-sm font-medium text-zinc-400">Task ID</th>
                        <th className="text-left py-3 px-4 text-sm font-medium text-zinc-400">Tarih</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-white/5">
                      {kieTasks.videos.map((video: any) => (
                        <tr key={video.id} className="hover:bg-zinc-800/50 transition-colors">
                          <td className="py-3 px-4 text-sm font-mono">#{video.id}</td>
                          <td className="py-3 px-4">
                            <div>
                              <p className="text-sm">{video.userName || `User #${video.userId}`}</p>
                              <p className="text-xs text-zinc-500">{video.userEmail}</p>
                            </div>
                          </td>
                          <td className="py-3 px-4">
                            <code className="text-xs bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded">
                              {video.model}
                            </code>
                          </td>
                          <td className="py-3 px-4">
                            <p className="text-sm text-zinc-400 truncate max-w-[200px]" title={video.prompt}>
                              {video.prompt}
                            </p>
                          </td>
                            ...
                            <td className="py-3 px-4">
                              {video.liveStatus ? (
                                <div className="flex items-center gap-2">
                                  ...
                                  {video.liveError && (
                                    <span className="text-xs text-red-400" title={video.liveError}>
                          ...
                          <td className="py-3 px-4 text-sm text-zinc-400">
                            {formatDistanceToNow(new Date(video.createdAt), { locale: tr, addSuffix: true })}
                          </td>
                        </tr>
                      ))}
                    </tbody>
                  ...
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-white/5">
                      {kieTasks.images.map((image: any) => (
                        <tr key={image.id} className="hover:bg-zinc-800/50 transition-colors">
                          <td className="py-3 px-4 text-sm font-mono">#{image.id}</td>
                          <td className="py-3 px-4">
                            <div>
                              <p className="text-sm">{image.userName || `User #${image.userId}`}</p>
                              <p className="text-xs text-zinc-500">{image.userEmail}</p>
                            </div>
                          </td>
                          <td className="py-3 px-4">
                            <code className="text-xs bg-purple-500/20 text-purple-400 px-2 py-0.5 rounded">
                              {image.model}
                            </code>
                          </td>
                          <td className="py-3 px-4">
                            <p className="text-sm text-zinc-400 truncate max-w-[200px]" title={image.prompt}>
                              {image.prompt}
                            </p>
                          </td>
                            ...
                            <td className="py-3 px-4">
                              {image.liveStatus ? (
                                <div className="flex items-center gap-2">
                                  ...
                                  {image.liveError && (
                                    <span className="text-xs text-red-400" title={image.liveError}>
                                      <AlertTriangle className="h-3 w-3" />
                                    </span>
                                  )}
                                </div>
                          ...
                          <td className="py-3 px-4 text-sm text-zinc-400">
                            {formatDistanceToNow(new Date(image.createdAt), { locale: tr, addSuffix: true })}
                          </td>
                        </tr>
                      ))}
                    </tbody>
          ...
          {(!kieTasks?.videos?.length && !kieTasks?.images?.length) && (
            <div className="text-center py-12 bg-zinc-900/50 rounded-2xl border border-white/10">
              ...
              <p className="text-zinc-500">Kie AI'da bekleyen işlem bulunamadı</p>
