  ...
  try {
    ...
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        console.log(`✅ Payment successful: Order ${orderId} - Added ${order.creditsAmount} credits to user ${order.userId}`);
      ...
      case "charge.refunded": {
        ...
        if (order && order.status === "success") {
          ...
          if (user) {
            ...
            console.log(`🔄 Refund processed for order ${order.id}: Deducted ${order.creditsAmount} credits from user ${order.userId}`);
          }
        }
        break;
      }
