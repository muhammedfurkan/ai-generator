...
import { upscaleHistory, users, creditTransactions } from "../../drizzle/schema";
import { eq, desc, and } from "drizzle-orm";
import { upscaleImage, getUpscaleStatus, calculateUpscaleCreditCost, type UpscaleFactor } from "../kieAiApi";
...
async function downloadAndUploadToS3(imageUrl: string, userId: number): Promise<string> {
...
export const upscaleRouter = router({
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const creditCost = calculateUpscaleCreditCost(upscaleFactor as UpscaleFactor);
      ...
      // Record transaction
      await db.recordCreditTransaction(userId, "deduct", creditCost, `Görsel upscale (${upscaleFactor}x)`);
      ...
      try {
        ...
        await db.refundCredits(userId, creditCost, `Upscale başarısız - ${upscaleFactor}x`);
        ...
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: error instanceof Error ? error.message : "Upscale işlemi başarısız oldu.",
        });
      }
    }),
  ...
  checkStatus: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (statusResult.status === "completed" && statusResult.imageUrl) {
        ...
        const s3Url = await downloadAndUploadToS3(statusResult.imageUrl, userId);
        ...
        if (record.creditsCost > 0) {
          console.log(`[Upscale] Task ${taskId} failed, refunding ${record.creditsCost} credits to user ${userId}`);
          await db.refundCredits(userId, record.creditsCost, `Upscale başarısız - ${record.upscaleFactor}x`);
        }
