...
import { eq, desc, asc, sql, and, gte, lte, like, count, or } from "drizzle-orm";
...
export const adminPanelRouter = router({
  ...
  getDashboardOverview: adminProcedure.query(async () => {
    ...
    const [videoCount] = await db.select({ count: count() }).from(videoGenerations);
    const [feedbackCount] = await db.select({ count: count() }).from(feedbacks).where(eq(feedbacks.status, "new"));
    ...
    const [todayImages] = await db.select({ count: count() })
      .from(generatedImages)
      .where(gte(generatedImages.createdAt, today));

    // Get total credits in circulation
    const [totalCredits] = await db.select({ total: sql<number>`SUM(credits)` }).from(users);

    // Get recent transactions sum
    const [recentRevenue] = await db.select({ total: sql<number>`SUM(amount)` })
  ...
  getAdvancedDashboardStats: adminProcedure
    ...
    .query(async ({ input }) => {
      ...
      const modelStatsWithErrorRate = modelStats.map(m => ({
        ...m,
        errorRate: m.totalRequests > 0
          ? ((m.failedRequests || 0) / m.totalRequests * 100).toFixed(2)
          : "0.00",
      }));

      // Sort by usage
      const mostUsedModels = [...modelStatsWithErrorRate].sort((a, b) => (b.totalRequests || 0) - (a.totalRequests || 0)).slice(0, 5);
      const mostFailingModels = [...modelStatsWithErrorRate].sort((a, b) => parseFloat(b.errorRate) - parseFloat(a.errorRate)).slice(0, 5);

      // Total API cost
      const totalApiCost = modelStats.reduce((sum, m) => sum + parseFloat(m.totalCostUsd || "0"), 0);
      ...
      const [periodImages] = await db.select({ count: count() })
        .from(generatedImages)
        .where(gte(generatedImages.createdAt, startDate));

      const [periodVideos] = await db.select({ count: count() })
  ...
  getVideos: adminProcedure
    ...
    .query(async ({ input }) => {
      ...
      // Get counts
      const [totalCount] = await db.select({ count: count() }).from(videoGenerations);
      const [completedCount] = await db.select({ count: count() }).from(videoGenerations).where(eq(videoGenerations.status, "completed"));
      const [processingCount] = await db.select({ count: count() }).from(videoGenerations).where(eq(videoGenerations.status, "processing"));
      const [failedCount] = await db.select({ count: count() }).from(videoGenerations).where(eq(videoGenerations.status, "failed"));
  ...
  getImages: adminProcedure
    ...
    .query(async ({ input }) => {
      ...
      // Get counts
      const [totalCount] = await db.select({ count: count() }).from(generatedImages);
      const [completedCount] = await db.select({ count: count() }).from(generatedImages).where(eq(generatedImages.status, "completed"));
      const [processingCount] = await db.select({ count: count() }).from(generatedImages).where(eq(generatedImages.status, "processing"));
      const [failedCount] = await db.select({ count: count() }).from(generatedImages).where(eq(generatedImages.status, "failed"));
    ...
    .query(async ({ input }) => {
      ...
      if (input.category) {
        return await db.select()
          .from(siteSettings)
          .where(eq(siteSettings.category, input.category as typeof siteSettings.category.enumValues[number]))
  ...
  updateSiteSetting: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (existing) {
        ...
        await logActivity(ctx.user.id, "setting.update", "siteSetting", existing.id, existing.value, input.value);
      }
  ...
  createSiteSetting: adminProcedure
    .input(z.object({
      key: z.string(),
      value: z.string().nullable(),
      category: z.enum(["general", "seo", "contact", "social", "email", "notification", "security", "maintenance"]),
      label: z.string(),
      description: z.string().optional(),
      inputType: z.enum(["text", "textarea", "number", "boolean", "url", "email", "json", "image"]).default("text"),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [result] = await db.insert(siteSettings).values(input).$returningId();

      await logActivity(ctx.user.id, "setting.create", "siteSetting", result.id, null, input);
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.features && input.features.trim() !== "") {
        try {
          ...
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Özellikler (Features) alanı geçerli bir JSON formatında olmalıdır (örn: [\"özellik 1\", \"özellik 2\"])",
          });
        }
      }

      const [result] = await db.insert(creditPackages).values(dataToInsert).$returningId();

      await logActivity(ctx.user.id, "package.create", "creditPackage", result.id, null, input);
  ...
  updateCreditPackage: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [existing] = await db.select().from(creditPackages).where(eq(creditPackages.id, input.id));
      ...
      const dataToUpdate = {
        ...updateData,
        originalPrice: updateData.originalPrice === "" ? null : updateData.originalPrice,
      ...
      await db.update(creditPackages).set(dataToUpdate).where(eq(creditPackages.id, id));

      await logActivity(ctx.user.id, "package.update", "creditPackage", id, existing, updateData);
  ...
  deleteCreditPackage: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logActivity(ctx.user.id, "package.delete", "creditPackage", input.id);
  ...
  createDiscountCode: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [existing] = await db.select().from(discountCodes).where(eq(discountCodes.code, input.code.toUpperCase()));
      if (existing) throw new TRPCError({ code: "CONFLICT", message: "Bu kod zaten mevcut" });
      ...
      await logActivity(ctx.user.id, "discount.create", "discountCode", result.id, null, input);
  ...
  updateDiscountCode: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logActivity(ctx.user.id, "discount.update", "discountCode", id, null, updateData);
  ...
  deleteDiscountCode: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logActivity(ctx.user.id, "discount.delete", "discountCode", input.id);
  ...
  getFeaturePricing: adminProcedure.query(async () => {
    const db = await getDb();
    if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Database not available" });
    return await db.select().from(featurePricing).orderBy(asc(featurePricing.category), asc(featurePricing.featureKey));
  }),
  ...
  initializeFeaturePricing: adminProcedure.mutation(async ({ ctx }) => {
    ...
    const defaultPricing = [
      ...
      { featureKey: "image_qwen_1k", featureName: "Görsel üretim (Qwen 1K)", category: "image", credits: 10, description: "Qwen modeli ile 1K çözünürlükte görsel üretimi" },
      { featureKey: "image_qwen_2k", featureName: "Görsel üretim (Qwen 2K)", category: "image", credits: 15, description: "Qwen modeli ile 2K çözünürlükte görsel üretimi" },
      { featureKey: "image_qwen_4k", featureName: "Görsel üretim (Qwen 4K)", category: "image", credits: 20, description: "Qwen modeli ile 4K çözünürlükte görsel üretimi" },

      // Image Generation - Nano Banana Pro
      { featureKey: "image_nano_banana_1k", featureName: "Görsel üretim (Nano Banana Pro 1K)", category: "image", credits: 12, description: "Nano Banana Pro ile 1K çözünürlükte görsel üretimi" },
      { featureKey: "image_nano_banana_2k", featureName: "Görsel üretim (Nano Banana Pro 2K)", category: "image", credits: 18, description: "Nano Banana Pro ile 2K çözünürlükte görsel üretimi" },
      { featureKey: "image_nano_banana_4k", featureName: "Görsel üretim (Nano Banana Pro 4K)", category: "image", credits: 25, description: "Nano Banana Pro ile 4K çözünürlükte görsel üretimi" },

      // Image Generation - SeeDream
      { featureKey: "image_seedream_basic", featureName: "Görsel üretim (SeeDream Basic)", category: "image", credits: 8, description: "SeeDream 4.5 ile 2K görsel üretimi" },
      { featureKey: "image_seedream_high", featureName: "Görsel üretim (SeeDream High)", category: "image", credits: 15, description: "SeeDream 4.5 ile 4K görsel üretimi" },

      // Video Generation - Veo 3.1
      { featureKey: "video_veo3_fast", featureName: "Video üretim (Veo 3.1 Hızlı)", category: "video", credits: 50, description: "Google Veo 3.1 hızlı mod - 8 saniye" },
      { featureKey: "video_veo3_quality", featureName: "Video üretim (Veo 3.1 Kalite)", category: "video", credits: 75, description: "Google Veo 3.1 kalite mod - 8 saniye" },

      // Video Generation - Grok
      { featureKey: "video_grok_standard", featureName: "Video üretim (Grok)", category: "video", credits: 15, description: "xAI Grok Imagine - 6 saniye" },

      // Video Generation - Kling
      { featureKey: "video_kling_5s", featureName: "Video üretim (Kling 5s)", category: "video", credits: 45, description: "Kling 2.5 Turbo - 5 saniye" },
      { featureKey: "video_kling_10s", featureName: "Video üretim (Kling 10s)", category: "video", credits: 75, description: "Kling 2.5 Turbo - 10 saniye" },

      // Video Generation - Sora 2
      { featureKey: "video_sora_10s", featureName: "Video üretim (Sora 10s)", category: "video", credits: 24, description: "OpenAI Sora 2 - 10 saniye" },
      { featureKey: "video_sora_15s", featureName: "Video üretim (Sora 15s)", category: "video", credits: 30, description: "OpenAI Sora 2 - 15 saniye" },

      // Upscale
      { featureKey: "upscale_2x", featureName: "Görsel upscale (2x)", category: "upscale", credits: 5, description: "2x çözünürlük artırma" },
      { featureKey: "upscale_4x", featureName: "Görsel upscale (4x)", category: "upscale", credits: 10, description: "4x çözünürlük artırma" },
      { featureKey: "skin_enhancement", featureName: "Cilt iyileştirme", category: "upscale", credits: 8, description: "AI cilt iyileştirme" },

      // AI Character
      { featureKey: "ai_character_create", featureName: "AI Karakter oluşturma", category: "ai_character", credits: 15, description: "Yeni AI karakter oluşturma" },
      { featureKey: "ai_character_generate", featureName: "AI Karakter görsel üretim", category: "ai_character", credits: 12, description: "Mevcut AI karakter ile görsel üretimi" },

      // Viral App
      { featureKey: "viral_app_basic", featureName: "Viral App (Basic)", category: "viral_app", credits: 20, description: "Basit viral app içeriği" },
      { featureKey: "viral_app_premium", featureName: "Viral App (Premium)", category: "viral_app", credits: 40, description: "Premium viral app içeriği" },
    ];
    ...
    for (const pricing of defaultPricing) {
      try {
        const existing = await db.select().from(featurePricing).where(eq(featurePricing.featureKey, pricing.featureKey)).limit(1);
        ...
        if (existing.length === 0) {
          ...
          await db.update(featurePricing)
    ...
    await logActivity(ctx.user.id, "pricing.initialize", undefined, undefined, undefined, { inserted, updated });
  ...
  updateFeaturePricing: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [existing] = await db.select().from(featurePricing).where(eq(featurePricing.id, input.id));
      if (!existing) throw new TRPCError({ code: "NOT_FOUND" });

      const { id, ...updateData } = input;
      await db.update(featurePricing).set(updateData).where(eq(featurePricing.id, id));

      await logActivity(ctx.user.id, "pricing.update", "featurePricing", id, existing.credits, input.credits);
  ...
  createFeaturePricing: adminProcedure
    .input(z.object({
      ...
      category: z.enum(["image", "video", "upscale", "ai_character", "viral_app", "multi_angle", "product_promo", "ugc_ad"]),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const existing = await db.select().from(featurePricing).where(eq(featurePricing.featureKey, input.featureKey)).limit(1);
      if (existing.length > 0) {
        throw new TRPCError({ code: "CONFLICT", message: "Bu feature key zaten mevcut" });
      }

      const [result] = await db.insert(featurePricing).values(input).$returningId();

      await logActivity(ctx.user.id, "pricing.create", "featurePricing", result.id, null, input);
  ...
  deleteFeaturePricing: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logActivity(ctx.user.id, "pricing.delete", "featurePricing", input.id);
  ...
  createAnnouncement: adminProcedure
    .input(z.object({
      ...
      targetAudience: z.enum(["all", "logged_in", "logged_out", "new_users"]).default("all"),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [result] = await db.insert(announcements).values(input).$returningId();

      await logActivity(ctx.user.id, "announcement.create", "announcement", result.id, null, input);
  ...
  updateAnnouncement: adminProcedure
    .input(z.object({
      ...
      type: z.enum(["popup", "banner", "notification", "maintenance"]).optional(),
      targetAudience: z.enum(["all", "logged_in", "logged_out", "new_users"]).optional(),
      buttonText: z.string().optional(),
      buttonUrl: z.string().optional(),
      imageUrl: z.string().optional(),
      backgroundColor: z.string().optional(),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logActivity(ctx.user.id, "announcement.update", "announcement", id, null, updateData);
  ...
  deleteAnnouncement: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logActivity(ctx.user.id, "announcement.delete", "announcement", input.id);
  ...
  createFaq: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logActivity(ctx.user.id, "faq.create", "faq", result.id, null, input);
  ...
  getViralAppsConfig: adminProcedure.query(async () => {
    const db = await getDb();
    if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Database not available" });
    return await db.select().from(viralAppsConfig).orderBy(asc(viralAppsConfig.sortOrder), asc(viralAppsConfig.name));
  }),
  ...
  updateViralAppConfig: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const { id, ...updateData } = input;
      await db.update(viralAppsConfig).set(updateData).where(eq(viralAppsConfig.id, id));

      await logActivity(ctx.user.id, "viralApp.update", "viralAppConfig", id, null, updateData);
  ...
  createViralAppConfig: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Check if already exists
      const existing = await db.select().from(viralAppsConfig).where(eq(viralAppsConfig.appKey, input.appKey)).limit(1);
      if (existing.length > 0) {
        throw new TRPCError({ code: "CONFLICT", message: "Bu app key zaten mevcut" });
      }

      const [result] = await db.insert(viralAppsConfig).values(input).$returningId();
      await logActivity(ctx.user.id, "viralApp.create", "viralAppConfig", result.id, null, input);
  ...
  deleteViralAppConfig: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logActivity(ctx.user.id, "viralApp.delete", "viralAppConfig", input.id);
  ...
  initializeViralApps: adminProcedure.mutation(async ({ ctx }) => {
    ...
    const defaultApps = [
      { appKey: "story_generator", name: "Hikaye Oluşturucu", emoji: "📖", description: "AI ile sosyal medya hikayeleri oluşturun", credits: 15, sortOrder: 1, promptTemplate: "Create a cinematic social media story for this image, engaging and viral style, high quality, 4k" },
      { appKey: "meme_maker", name: "Meme Yapıcı", emoji: "😂", description: "Viral meme'ler oluşturun", credits: 10, sortOrder: 2, isPopular: true, promptTemplate: "Turn this image into a funny viral meme video, humorous atmosphere, internet culture style" },
      { appKey: "quote_designer", name: "Alıntı Tasarımcısı", emoji: "💬", description: "İlham verici alıntı görselleri", credits: 8, sortOrder: 3, promptTemplate: "Add an inspirational quote overlay to this image with beautiful typography, cinematic lighting, elegant style" },
      { appKey: "product_showcase", name: "Ürün Showcase", emoji: "🛍️", description: "E-ticaret ürün görselleri", credits: 20, sortOrder: 4, promptTemplate: "Showcase this product in a professional studio setting with dynamic lighting, 4k, advertising style" },
      { appKey: "before_after", name: "Önce & Sonra", emoji: "🔄", description: "Karşılaştırmalı görseller", credits: 15, sortOrder: 5, promptTemplate: "Create a transformation video showing before and after states, smooth transition, high quality" },
      { appKey: "carousel_creator", name: "Carousel Oluşturucu", emoji: "📱", description: "Instagram carousel postları", credits: 25, sortOrder: 6, isPopular: true, promptTemplate: "Create a seamless scrolling carousel video from this image, engaging motion, social media trend" },
      { appKey: "thumbnail_maker", name: "Thumbnail Yapıcı", emoji: "🎬", description: "YouTube thumbnail görselleri", credits: 12, sortOrder: 7, promptTemplate: "Create an eye-catching YouTube thumbnail motion poster, vibrant colors, high contrast, clickbait style" },
      { appKey: "avatar_generator", name: "Avatar Oluşturucu", emoji: "👤", description: "Kişisel AI avatarları", credits: 18, sortOrder: 8, promptTemplate: "Animate this avatar looking around and smiling, realistic facial expressions, high detail, 8k" },
    ];
    ...
    for (const app of defaultApps) {
      try {
        const existing = await db.select().from(viralAppsConfig).where(eq(viralAppsConfig.appKey, app.appKey)).limit(1);
        if (existing.length === 0) {
    ...
    await logActivity(ctx.user.id, "viralApp.initialize", undefined, undefined, undefined, { inserted });
  ...
  getUserDetails: adminProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [imageCount] = await db.select({ count: count() })
        .from(generatedImages)
  ...
  updateUserStatus: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logActivity(ctx.user.id, "user.update", "user", userId, existing, updateData);
  ...
  updateFeedbackStatus: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logActivity(ctx.user.id, "feedback.update", "feedback", id, null, updateData);
  ...
  toggleCharacterPublic: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logActivity(ctx.user.id, "character.moderate", "aiCharacter", input.id, null, { isPublic: input.isPublic });
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      for (const userId of input.userIds) {
        try {
          ...
          results.push({ userId, success: true, newBalance: user.credits + input.amount });
        } catch (error) {
          results.push({ userId, success: false, error: String(error) });
        }
      }

      await logActivity(ctx.user.id, "credit.bulkAdd", "user", undefined, null, { userIds: input.userIds, amount: input.amount });
  ...
  getBannedUsers: adminProcedure.query(async () => {
    ...
    return await db.select({
      ...
      createdAt: users.createdAt,
    }).from(users).where(eq(users.isBanned, true)).orderBy(desc(users.bannedAt));
  }),
  ...
  banUser: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logActivity(ctx.user.id, "user.ban", "user", input.userId, null, { reason: input.reason });
  ...
  createUser: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logActivity(ctx.user.id, "user.create", "user", result.id, null, { name: input.name, email: input.email, role: input.role });
  ...
  addBannedIp: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logActivity(ctx.user.id, "ip.ban", "bannedIp", undefined, null, input);
  ...
  getBannedEmails: adminProcedure.query(async () => {
    ...
    return await db.select().from(bannedEmails).orderBy(desc(bannedEmails.createdAt));
  }),
  ...
  addBannedEmail: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logActivity(ctx.user.id, "email.ban", "bannedEmail", undefined, null, input);
  ...
  updateHomepageSection: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logActivity(ctx.user.id, "homepage.updateSection", "homepageSection", input.id, null, updateData);
  ...
  getAiModels: adminProcedure.query(async () => {
    ...
    return await db.select().from(aiModelConfig).orderBy(asc(aiModelConfig.modelName));
  }),
  ...
  initializeAiModels: adminProcedure.mutation(async ({ ctx }) => {
    ...
    const defaultModels = [
      // Image Generation Models
      {
        ...
        priority: 1,
        description: "Ana görsel üretim modeli. Yüksek kaliteli, hızlı sonuçlar.",
        costPerRequest: "0.02",
      },
      {
        modelKey: "qwen",
        ...
        premiumUserDailyLimit: 100,
        priority: 2,
        description: "Alternatif görsel modeli. Referans görsel desteği mevcut.",
      ...
      // Video Generation Models
      {
        ...
        priority: 1,
        description: "Google'ın en gelişmiş video modeli. Yüksek kalite, sesli video desteği.",
        costPerRequest: "0.50",
      },
      {
        modelKey: "sora2",
        ...
        premiumUserDailyLimit: 30,
        priority: 2,
        description: "OpenAI'in güçlü video üretim modeli. Uzun süreli videolar.",
        costPerRequest: "0.40",
      },
      {
        modelKey: "kling",
    ...
    for (const model of defaultModels) {
      try {
        // Check if exists
        const existing = await db.select().from(aiModelConfig).where(eq(aiModelConfig.modelKey, model.modelKey)).limit(1);
    ...
    await logActivity(ctx.user.id, "models.initialize", undefined, undefined, undefined, { inserted, updated });
  ...
  createAiModel: adminProcedure
    .input(z.object({
      ...
      provider: z.string(),
      isActive: z.union([z.boolean(), z.number()]).transform(v => !!v).default(true),
      isMaintenanceMode: z.union([z.boolean(), z.number()]).transform(v => !!v).default(false),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logActivity(ctx.user.id, "model.create", "aiModelConfig", result.insertId, null, input);
  ...
  updateAiModel: adminProcedure
    .input(z.object({
      ...
      isMaintenanceMode: z.union([z.boolean(), z.number()]).transform(v => !!v).optional(),
      maxResolutionWidth: z.number().optional(),
      maxResolutionHeight: z.number().optional(),
      maxVideoDurationSeconds: z.number().nullable().optional(),
      freeUserDailyLimit: z.number().optional(),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const { id, ...updateData } = input;
      await db.update(aiModelConfig).set(updateData).where(eq(aiModelConfig.id, id));

      await logActivity(ctx.user.id, "model.update", "aiModelConfig", id, null, updateData);
      return { success: true };
    }),
  ...
  toggleAiModel: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await db.update(aiModelConfig).set({ isActive: input.isActive }).where(eq(aiModelConfig.id, input.id));
      await logActivity(ctx.user.id, "model.toggle", "aiModelConfig", input.id, null, { isActive: input.isActive });
      return { success: true };
    }),
  ...
  getPromptBlacklist: adminProcedure.query(async () => {
    ...
    return await db.select().from(promptBlacklist).orderBy(desc(promptBlacklist.createdAt));
  }),

  createPromptBlacklist: adminProcedure
    .input(z.object({
      pattern: z.string(),
      patternType: z.enum(["exact", "contains", "regex", "starts_with", "ends_with"]),
      category: z.enum(["nsfw", "spam", "abuse", "illegal", "copyright", "other"]),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logActivity(ctx.user.id, "blacklist.create", "promptBlacklist", result.insertId, null, input);
      return { success: true, id: result.insertId };
    }),
  ...
  updatePromptBlacklist: adminProcedure
    .input(z.object({
      ...
      patternType: z.enum(["exact", "contains", "regex", "starts_with", "ends_with"]).optional(),
      category: z.enum(["nsfw", "spam", "abuse", "illegal", "copyright", "other"]).optional(),
      severity: z.enum(["low", "medium", "high", "critical"]).optional(),
      actionType: z.enum(["block", "warn", "flag_for_review", "auto_ban"]).optional(),
      warningMessage: z.string().optional(),
      isActive: z.boolean().optional(),
    }))
    .mutation(async ({ ctx, input }) => {
      ...
      const { id, ...updateData } = input;
      await db.update(promptBlacklist).set(updateData).where(eq(promptBlacklist.id, id));

      await logActivity(ctx.user.id, "blacklist.update", "promptBlacklist", id, null, updateData);
      return { success: true };
    }),
  ...
  deletePromptBlacklist: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logActivity(ctx.user.id, "blacklist.delete", "promptBlacklist", input.id);
      return { success: true };
    }),
  ...
  reviewFlaggedPrompt: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await db.update(flaggedPrompts).set({
        status: input.status,
      ...
      if (input.banUser && input.status === "banned") {
        const [flagged] = await db.select().from(flaggedPrompts).where(eq(flaggedPrompts.id, input.id));
        if (flagged) {
          await db.update(users).set({
            isBanned: true,
            bannedAt: new Date(),
            banReason: "Prompt abuse: " + (input.reviewNotes || "Tekrarlanan ihlal"),
            bannedBy: ctx.user.id,
          }).where(eq(users.id, flagged.userId));
        }
      }

      await logActivity(ctx.user.id, "prompt.review", "flaggedPrompts", input.id, null, input);
      return { success: true };
    }),
  ...
  getKieApiTasks: adminProcedure
    ...
    .query(async ({ input }) => {
        ...
        for (const video of videoTasks.slice(0, 10)) {
          if (video.taskId) {
            try {
              ...
              video.liveError = e instanceof Error ? e.message : "API check failed";
            }
          }
        }
          ...
          if (image.taskId) {
            try {
              ...
              if (response.code === 200 && response.data) {
                ...
                if (state === "waiting") image.liveStatus = "processing";
              ...
              image.liveError = e instanceof Error ? e.message : "API check failed";
            }
          }
        }
      }
      ...
      const stats = {
        ...
        processingVideos: pendingVideos.filter(v => v.status === "processing").length,
        pendingImages: pendingImages.filter(i => i.status === "pending").length,
        processingImages: pendingImages.filter(i => i.status === "processing").length,
  ...
  getPaymentRecords: adminProcedure
    ...
    .query(async ({ input }) => {
      ...
      const paymentRecordsData = await db.select({
      ...
      const { stripeOrders, creditPackages } = await import("../../drizzle/schema");
      const stripeOrdersData = await db.select({
        id: stripeOrders.id,
      ...
      // Merge both arrays
      const allPayments = [
        ...paymentRecordsData.map(p => ({ ...p, source: 'paymentRecords' as const })),
        ...stripeOrdersData.map(s => ({ ...s, source: 'stripeOrders' as const })),
      ];
      ...
      const paginatedPayments = allPayments.slice(input.offset, input.offset + input.limit);
  ...
  getPaymentStats: adminProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [paymentRecordsStats] = await db.select({
        totalRevenue: sql<number>`COALESCE(SUM(CASE WHEN status = 'completed' THEN amount ELSE 0 END), 0)`,
      ...
      const [stripeStats] = await db.select({
      ...
      // Combine stats
      const stats = {
        totalRevenue: (paymentRecordsStats.totalRevenue || 0) + (stripeStats.totalRevenue || 0),
        totalRevenueUsd: paymentRecordsStats.totalRevenueUsd || 0,
        successfulCount: (paymentRecordsStats.successfulCount || 0) + (stripeStats.successfulCount || 0),
        totalCount: (paymentRecordsStats.totalCount || 0) + (stripeStats.totalCount || 0),
        refundedAmount: (paymentRecordsStats.refundedAmount || 0) + (stripeStats.refundedAmount || 0),
        refundedCount: (paymentRecordsStats.refundedCount || 0) + (stripeStats.refundedCount || 0),
      };

      const successRate = stats.totalCount > 0 ? stats.successfulCount / stats.totalCount : 0;

      // Package breakdown from paymentRecords
      const packageBreakdownPayments = await db.select({
        packageId: paymentRecords.packageId,
        packageName: paymentRecords.packageName,
        credits: paymentRecords.credits,
        ...
        .groupBy(paymentRecords.packageId, paymentRecords.packageName, paymentRecords.credits);

      // Package breakdown from stripeOrders
      const { creditPackages } = await import("../../drizzle/schema");
      const packageBreakdownStripe = await db.select({
        packageId: stripeOrders.packageId,
        packageName: creditPackages.name,
        credits: stripeOrders.creditsAmount,
        saleCount: sql<number>`COUNT(*)`,
        ...
        .groupBy(stripeOrders.packageId, creditPackages.name, stripeOrders.creditsAmount);
      ...
      [...packageBreakdownPayments, ...packageBreakdownStripe].forEach(pkg => {
        if (pkg.packageId) {
          ...
          if (existing) {
            ...
            existing.totalRevenue = Number(existing.totalRevenue) + Number(pkg.totalRevenue);
          } else {
      ...
      const methodBreakdownPayments = await db.select({
      ...
      const stripeMethodStats = await db.select({
        count: sql<number>`COUNT(*)`,
  ...
  refundPayment: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logActivity(ctx.user.id, "payment.refund", "paymentRecords", input.id);
      return { success: true };
    }),
  ...
  getFraudAlerts: adminProcedure.query(async () => {
    ...
    const duplicateCards = await db.select({
  ...
  createAdminRole: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logActivity(ctx.user.id, "role.create", "adminRoles", result.insertId, null, input);
      return { success: true, id: result.insertId };
    }),
  ...
  updateAdminRole: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logActivity(ctx.user.id, "role.update", "adminRoles", id, null, updateData);
      return { success: true };
    }),
  ...
  getAdminUsers: adminProcedure.query(async () => {
    ...
    const simpleAdmins = await db.select({
      id: users.id,
      userId: users.id,
      userName: users.name,
      userEmail: users.email,
    ...
    return [...admins, ...simpleAdmins.filter(a => !admins.find(e => e.userId === a.id))];
  }),
  ...
  getRateLimits: adminProcedure.query(async () => {
    ...
    return await db.select().from(systemRateLimits).orderBy(asc(systemRateLimits.limitKey));
  }),
  ...
  updateRateLimit: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const { id, ...updateData } = input;
      await db.update(systemRateLimits).set(updateData).where(eq(systemRateLimits.id, id));

      await logActivity(ctx.user.id, "rateLimit.update", "systemRateLimits", id, null, updateData);
      return { success: true };
    }),
  ...
  getAiInfluencerSettings: adminProcedure.query(async () => {
    ...
    try {
      ...
      return {
        ...
        defaultModel: settings.ai_influencer_default_model || defaults.defaultModel,
        isNanoBananaEnabled: settings.ai_influencer_nano_banana_enabled !== "false",
        isQwenEnabled: settings.ai_influencer_qwen_enabled !== "false",
        isSeedreamEnabled: settings.ai_influencer_seedream_enabled !== "false",
      };
    } catch (error) {
      console.error("[AdminPanel] Error fetching AI Influencer settings:", error);
      return defaults;
    }
  }),
  ...
  updateAiInfluencerSettings: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const settingsToSave = [
        { key: "ai_influencer_nano_banana_pricing", value: JSON.stringify(input.nanoBananaPricing), label: "Nano Banana Fiyatlandırma" },
        { key: "ai_influencer_qwen_pricing", value: JSON.stringify(input.qwenPricing), label: "Qwen Fiyatlandırma" },
        { key: "ai_influencer_seedream_pricing", value: JSON.stringify(input.seedreamPricing), label: "SeeDream Fiyatlandırma" },
        { key: "ai_influencer_default_model", value: input.defaultModel || "nano-banana-pro", label: "Varsayılan Model" },
        { key: "ai_influencer_nano_banana_enabled", value: input.isNanoBananaEnabled ? "true" : "false", label: "Nano Banana Aktif" },
        { key: "ai_influencer_qwen_enabled", value: input.isQwenEnabled ? "true" : "false", label: "Qwen Aktif" },
        { key: "ai_influencer_seedream_enabled", value: input.isSeedreamEnabled ? "true" : "false", label: "SeeDream Aktif" },
      ];

      try {
        console.log("[AdminPanel] Saving AI Influencer settings:", settingsToSave.map(s => ({ key: s.key, value: s.value })));
        ...
        await logActivity(ctx.user.id, "aiInfluencer.updateSettings", undefined, undefined, null, input);

        console.log("[AdminPanel] AI Influencer settings saved successfully");
        return { success: true };
      } catch (error) {
        console.error("[AdminPanel] Error updating AI Influencer settings:", error);
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Ayarlar kaydedilemedi" });
      }
    }),
});
