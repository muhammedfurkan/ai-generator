...
export async function createNotification(params: {
  ...
  try {
    ...
    console.log(
      `[Notification] Created notification for user ${params.userId}: ${params.title}`
    );
    return true;
  } catch (error) {
    ...
    const recentNotification = await db
      ...
      .where(
        and(
          ...
          eq(notifications.type, "low_credits"),
          sql`${notifications.createdAt} > DATE_SUB(NOW(), INTERVAL 24 HOUR)`
        )
      )
      .limit(1);
